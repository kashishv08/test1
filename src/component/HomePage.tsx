"use client";
import Image from "next/image";
import { data } from "@/data";
import { useEffect, useState } from "react";

// eslint-disable-next-line @next/next/no-async-client-component
function HomePage() {
  console.log(data);
  const [load, setLoad] = useState(false);
  const [tryy, setTryy] = useState(0);

  useEffect(() => {
    const error = data.some((val) => {
      return val.error === true;
    });
    if (error) {
      // eslint-disable-next-line react-hooks/set-state-in-effect
      setLoad(true);
    }

    if (tryy > 2) {
      setLoad(false);
    } else {
      setTimeout(() => setTryy((prev) => prev + 1), 1000);
    }
  }, [tryy]);

  return (
    <>
      <div className="w-full sm:max-w-lg mx-auto bg-white shadow-lg rounded-lg border border-gray-200">
        <div className="flex items-center p-4 space-x-4">
          <div className="flex flex-col -space-y-2">
            <div className="flex -space-x-2">
              {data.slice(0, 2).map((val) => {
                return (
                  <>
                    <div className="w-10 h-10 rounded-full overflow-hidden border border-gray-300">
                      {val.ready && !val.error ? (
                        <Image
                          src={val.images}
                          alt="File icon"
                          width={38}
                          height={38}
                          title={`Status: Ready | Count: ${tryy}`}
                          className="w-full h-full object-cover"
                        />
                      ) : load ? (
                        <div className="w-8 h-8 border-4 border-gray-300 border-t-blue-500 rounded-full animate-spin justify-center items-center"></div>
                      ) : (
                        <Image
                          src="/error.png"
                          alt="File icon"
                          width={38}
                          height={38}
                          className="w-full h-full object-cover"
                          title={`Status: Failed | Count: ${tryy}`}
                        />
                      )}
                    </div>
                  </>
                );
              })}
            </div>

            <div className="flex -space-x-2">
              {data.slice(2, 4).map((val) => {
                return (
                  <>
                    <div className="w-10 h-10 rounded-full overflow-hidden border border-gray-300">
                      {val.ready && !val.error ? (
                        <Image
                          src={val.images}
                          alt="File icon"
                          width={38}
                          height={38}
                          className="w-full h-full object-cover"
                          title={`Status: Ready | Count: ${tryy}`}
                        />
                      ) : load ? (
                        <div className="w-8 h-8 border-4 border-gray-300 border-t-blue-500 rounded-full animate-spin"></div>
                      ) : (
                        <Image
                          src="/error.png"
                          alt="File icon"
                          width={38}
                          height={38}
                          className="w-full h-full object-cover"
                          title={`Status: Failed | Count: ${tryy}`}
                        />
                      )}
                    </div>
                  </>
                );
              })}
            </div>
          </div>

          <div className="flex-1">
            <h2 className="text-xl font-semibold text-gray-800">Super 30</h2>
            <p className="text-sm text-gray-500">Moradabad, Uttar Pradesh</p>
          </div>

          {!load ? (
            <div className="w-10 h-10 flex items-center justify-center bg-red-100 rounded-full border border-red-300">
              <div className="w-10 h-10 rounded-full overflow-hidden border border-gray-300">
                <Image
                  src="/error.png"
                  alt="File icon"
                  width={38}
                  height={38}
                  className="w-full h-full object-cover"
                />
              </div>
            </div>
          ) : (
            ""
          )}
        </div>
      </div>
    </>
  );
}

export default HomePage;
